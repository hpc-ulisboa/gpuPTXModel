...
def readExecTimeKernels(data_path, bench, gpu):
            ...
            if not file.startswith("output_nvprof_metrics_"):
                ...
                with open('%s/%s/%s' %(data_path, bench, file)) as csvfile:
                    ...
                    for row_id, row in enumerate(spamreader):
                        ...
                        #             num_calls.append(float(row[2]))
                        #             avg_time_call.append(float(row[3]))
                        #             max_time_call.append(float(row[5]))
                        # else:
                        if row_id>4:
                            # print(row)
                            # sys.exit()
    ...
    # print(total_execution_all_kernels)
    # sys.exit()
...
def readNvprofFile(data_path, bench, gpu, list_event_names, time_per_kernel):
                        ...
                        if 'overflow' in row[0]:
                            ...
                            if event in list_event_names:
                                ...
                                if 'utilization' in event:
                                    list_events[event][kernel] = int(row[7].split()[1][1:-1])
                                else:
                                    # print(row[7])
                                    aux_value = float((re.findall('\d+\.\d+', row[7] ))[0])
                                    if  'GB/s' in row[7]:
    ...
    for event in list_event_names:
        if event not in list_events.keys():
            print(list_events)
            print('Missing values for event \'%s\' (possible overflow) for benchmarks: %s' %(event, bench))
            sys.exit()
    ...
    for event_id,event in enumerate(list_event_names):
        ...
        for kernel_name in time_per_kernel.keys():
            aggregated_list[event] += time_per_kernel[kernel_name] * list_events[event][kernel_name]
...
def readListsData(benchs, clocks, benchs_data_path, gpu_name, idle_powers):
    ...
    for bench_id, bench in enumerate(benchs):
        list_pow[bench_id] = np.zeros((num_mem_clocks, max_num_core_clocks), dtype=np.float32)
        list_time[bench_id] = np.zeros((num_mem_clocks, max_num_core_clocks), dtype=np.float32)
        list_energy[bench_id] = np.zeros((num_mem_clocks, max_num_core_clocks), dtype=np.float32)

        for clock_mem_id, clock_mem in enumerate(mem_clocks):
            for clock_core_id, clock_core in enumerate(core_clocks[clock_mem_id]):
                avg_pow_bench = rf.readPowerBench(benchs_data_path, '%s/%d/%d' %(bench, clock_mem, clock_core), idle_powers[clock_mem_id][clock_core_id])
                time_bench = rf.readExecTime(benchs_data_path, '%s/%d/%d' %(bench, clock_mem, clock_core), gpu_name)
                list_pow[bench_id][clock_mem_id, clock_core_id] = avg_pow_bench
                list_time[bench_id][clock_mem_id, clock_core_id] = (time_bench/1000.0)
                list_energy[bench_id][clock_mem_id, clock_core_id] = avg_pow_bench*(time_bench/1000.0)
...
def printListsData(benchs, clocks, lists, energy_mode):
    ...
    for bench_id, bench in enumerate(benchs):
        if bench_id == 0:
            header_line = '{message: >{width}}'.format(message='Clock Mem|', width=maxwidth+21)
            for clock_core_id, clock_core in enumerate(core_clocks[0]):
                if clock_core_id > 0:
                    header_line += '|'
                if energy_mode == True:
                    header_line += '{clock: >{width}}'.format(clock='%d MHz'%clock_core, width=11)
                else:
                    header_line += '{clock: >{width}}'.format(clock='%d MHz'%clock_core, width=13)
            print(header_line)
        bench_line = '{message: >{width}}: '.format(message=bench, width=maxwidth+2)
        for clock_mem_id, clock_mem in enumerate(mem_clocks):
            if clock_mem_id > 0:
                bench_line += '\n{message: >{width}}'.format(message="%s MHz| "%clock_mem, width=maxwidth+22)
            else:
            ...
            for clock_core_id, clock_core in enumerate(core_clocks[clock_mem_id]):
                ...
                if energy_mode == True:
                    bench_line += '{energy:8.1f} J'.format(energy=list_energy[bench_id][clock_mem_id, clock_core_id])
                else:
                    bench_line += '{time:6.1f},{power:5.1f}'.format(time=list_time[bench_id][clock_mem_id, clock_core_id], power=list_pow[bench_id][clock_mem_id, clock_core_id])
        print(bench_line)
        ...
        for clock_mem_id, clock_mem in enumerate(mem_clocks):
            ...
            if (type == 0) or (type == 1 and good_bench == True) or (type == 2 and good_bench == False):
                if normalized_t == True:
                    axis_t.plot(core_clocks[clock_mem_id], list_data_time[bench_id][clock_mem_id,:]/list_data_time[bench_id][-1,-1], linestyle='--', label=bench)
                else:
                    axis_t.plot(core_clocks[clock_mem_id], list_data_time[bench_id][clock_mem_id,:], linestyle='--', label=bench)
                if normalized_p == True:
                    axis_p.plot(core_clocks[clock_mem_id], list_data_pow[bench_id][clock_mem_id,:]/list_data_pow[bench_id][-1,-1], linestyle='--', label=bench)
                else:
                    axis_p.plot(core_clocks[clock_mem_id], list_data_pow[bench_id][clock_mem_id,:], linestyle='--', label=bench)
                if normalized_e == True:
                    axis_e.plot(core_clocks[clock_mem_id], list_data_energy[bench_id][clock_mem_id,:]/list_data_energy[bench_id][-1,-1], linestyle='--', label=bench)
                else:
...
def main():
    ...
    if test_data_path != '':
        ...
        lists_data_testbench = readListsData(test_benchmarks, clocks, test_data_path, gpu_name, idle_powers)
    ...
    if verbose == True:
        ...
        if clocks['core_clocks'].count(clocks['core_clocks'][0]) == len(clocks['core_clocks']):
            ...
            if test_data_path != '':
                print("\n\n=============================Test Benchmarks=============================\n")
                printListsData(test_benchmarks, clocks, lists_data_testbench, energy_mode)
        else:
    ...
    plotValues('micro_%s' %gpu_name, lists_data_ubench, clocks, ubenchmarks, normalized_t, normalized_p, normalized_e, type)
    plotValues('test_%s' %gpu_name, lists_data_testbench, clocks, test_benchmarks, normalized_t, normalized_p, normalized_e, type)

    if create_output_file == True:
        writeOutputFile(benchs_data_path, lists_data_ubench, ubenchmarks, clocks, gpu_name)
        writeOutputFile(test_data_path, lists_data_testbench, test_benchmarks, clocks, gpu_name)
